...
const Companies: React.FC<Props> = ({ lang }) => {
  const { hero, intro } = COMPANIES_PAGE_CONTENT;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
           ...
           <div className="grid md:grid-cols-2 gap-10">
               {Object.values(SUBSIDIARIES).map((sub, idx) => (
                   <div key={sub.id} className={`flex flex-col rounded-2xl overflow-hidden shadow-xl border border-slate-100 hover:shadow-2xl transition-all duration-500 group bg-white ${idx % 2 === 0 ? 'md:translate-y-8' : ''}`}>
                       <div className="h-64 relative overflow-hidden">
                           <div className="absolute inset-0 bg-gradient-to-t from-hewer-blue/80 to-transparent opacity-60 z-10"></div>
                           <img src={sub.heroImage} alt={sub.name[lang]} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700" />
                           <div className="absolute bottom-0 left-0 p-6 z-20">
                                <div className="bg-white p-3 rounded-lg inline-block shadow-lg mb-2">
                                     <img src={sub.logo} alt="logo" className="h-8 object-contain" />
                                </div>
                           </div>
                       </div>
